  ...
  jQuery.ajax({
    ...
    success:function(data){
        if(!data.redirect)
        {
          ...
          if(data.css)
          {
            var pageCssId  = page.attr('id')+'_css',
                cssElement = jQuery('#' + pageCssId, page);
            cssElement.replaceWith($('<style id="'+ pageCssId +'" type="text/css">' + Base64.decode(data.css) + '</style>'));
          }
    ...
    // (if error) call AjaxCallError event
    error:function(){
    ...
    complete:function(){
      jQuery.mobile.hidePageLoadingMsg();
      var page=jQuery.mobile.activePage.attr('id'),
          callback=page+'JSAjaxCallComplete';
...
function processPage(){
    ...
    // MEdit, MTextArea
    fixedsize.filter('input[type=text],input[type=number], input[type=password],input[type=password],textarea, input[type=true]').addClass('no-margin border-box');
